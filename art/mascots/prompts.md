...
5. **Позы стоят не на своих местах** — например, «машет» оказался внизу. Перерисовывать не нужно:
   запишите настоящий порядок в `art/mascots/order.json`, слева направо и сверху вниз:
   `{"ryazan-vdv": ["cheer", "point", "hello", "shrug"]}`. Для листов с порядком по умолчанию
