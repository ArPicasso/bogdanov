...
**26.09.2026, фэнтези.** Владелец попросил фэнтези. Раньше оно было в списке «сознательно не
делаем». Предложение — ADR-013, фэнтези «Звено». Устройство:
- ворота клуба и два звена из наклеек игроков;
- очки только из протоколов лиги;
- стоимость — публичная формула без спроса;
- денег в контуре нет.

Для «Звена» нужен сервер с базой и авторизацией `initData` — это этап 2 из таблицы выше, и
теперь его придётся делать. Как пришли к механике — `docs/research/2026-09-26-fantasy-zveno.md`.

Чего в плане сознательно нет: видео, билетов и мобильного приложения. Это разные бизнесы сверх того, что мы строим.
...
Фэнтези по КХЛ существует на [Sports.ru](https://www.sports.ru/fantasy/hockey/tournament/107.html): 17 игроков, бюджет 20 000, не более 3 из одного клуба. Для МХЛ и РХЛ фэнтези не найдено. Требует пофамильной статистики, поэтому стоит после этапа 5. Наша механика — ADR-013.
